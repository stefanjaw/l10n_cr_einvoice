...
class productProduct(models.Model):
    ...
    def _compute_fe_codigo_comercial_tipo(self):
        for record in self:
            if len(record.product_tmpl_id.product_variant_ids) == 1:
                record.fe_codigo_comercial_tipo = record.product_tmpl_id.fe_codigo_comercial_tipo or False

    def _set_fe_codigo_comercial_tipo(self):
        if len(self.product_tmpl_id.product_variant_ids) == 1:
            self.product_tmpl_id.fe_codigo_comercial_tipo = self.fe_codigo_comercial_tipo or False
