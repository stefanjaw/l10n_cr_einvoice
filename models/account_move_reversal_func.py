...
class AccountMoveReversal(models.TransientModel):
    ...
    def _prepare_default_reversalxxxxx(self, move):
        _logger.info(f"DEF18 ====== self: {self} move: {move} journal_id: {self.journal_id and self.journal_id.id or move.journal_id.id}\n\n")
        return {
            'ref': _('Reversal of: %s, %s') % (move.name, self.reason) if self.reason else _('Reversal of: %s') % (move.name),
            'date': self.date or move.date,
            'invoice_date': move.is_invoice(include_receipts=True) and (self.date or move.date) or False,
            'journal_id': self.journal_id.id and self.journal_id.id or move.journal_id.id,
